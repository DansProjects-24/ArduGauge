 ...
 *  Test code for using an analogue input with a lookup table to get a value
 *  
 *  1. Read analogue input
...
//Decalre Variables
int analogIn;   //If needed we can store raw analog value (0-1023)
...
Vin = (analogRead(potPin)/1023.0)*5.0; //Convert the analog input to the voltage range 0-5v.

float closestDiff = 5.0;    //Store lookuptable value which is closest. Start at 5v, can never be further than this
...
Temp = interpolate(Vin, LkUp_Col_1[closestIndex], LkUp_Col_1[boundingIndex], LkUp_Col_2[closestIndex], LkUp_Col_2[boundingIndex]);
